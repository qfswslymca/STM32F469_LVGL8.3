...
void show_array(int8_t *matrix)
{
    for (int i = 0; i < 28; i++)
    {
        for (int j = 0; j < 28; j++)
        {
            printf("%3d ", matrix[i*28+j]);
        }
        printf("\n");
    }
}
    ...
    for (int i = -2; i <= 2; i++)
    {
        for (int j = -2; j <= 2; j++)
        {
            if ((i*i + j*j) <= 4)
            {
                int r_x = x + i;
                int r_y = y + j;
                r_x = MIN(MAX(r_x, 0), 28);
                r_y = MIN(MAX(r_y, 0), 28);
                matrix[r_y*28+r_x] = 127;
            }
        }
    }
    //[y*28+x] = 255;
...
////        lv_chart_hide_series(chart, lv_chart_get_series_next(chart, NULL), FALSE);
...
//    lv_obj_add_event_cb(touch_num_screen, event_cb, LV_EVENT_RELEASED, touch_num_screen);
    ...
    lv_label_set_text(main_title, "Handwritting Recognize");
//    lv_obj_set_style_bg_color(main_title, lv_color_hex(0xff0000), LV_STATE_DEFAULT);
//    lv_obj_set_style_bg_opa(main_title, 100, LV_STATE_DEFAULT);
//    lv_obj_set_style_text_font(main_title, &lv_font_montserrat_30, LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(main_title, lv_color_hex(0xff0000), LV_STATE_DEFAULT);
//    lv_obj_set_pos(main_title,     10 , 0);
    lv_obj_align_to(main_title, touch_num_screen, LV_ALIGN_OUT_TOP_MID, 0, -10);
